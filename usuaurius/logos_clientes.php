<section class="client-logo ptb-100">
    <div class="container">
        <div class="row">
            <div class="col-md-2 col-sm-4 col-xs-6 section-margin">
                <a href="#"><img src="../img/logo-minedu.jpg" alt="Image"></a>
            </div>
            <div class="col-md-2 col-sm-4 col-xs-6 section-margin">
                <a href="#"><img src="../img/logo-minedu.jpg" alt="Image"></a>
            </div>
            <div class="col-md-2 col-sm-4 col-xs-6 section-margin">
                <a href="#"><img src="../img/logo-minedu.jpg" alt="Image"></a>
            </div>
            <div class="col-md-2 col-sm-4 col-xs-6 section-margin">
                <a href="#"><img src="../img/logo-minedu.jpg" alt="Image"></a>
            </div>
            <div class="col-md-2 col-sm-4 col-xs-6 section-margin">
                <a href="#"><img src="../img/logo-minedu.jpg" alt="Image"></a>
            </div>
            <div class="col-md-2 col-sm-4 col-xs-6 section-margin">
                <a href="#"><img src="../img/logo-minedu.jpg" alt="Image"></a>
            </div>
        </div>
    </div>
</section>

<section class="section-content-left-icon" style="font-weight: bolder;">
    <div class="container">
        <div class="row">
            <div class="col-md-3">
              <div class="left-icon-wraper">
                <a href="#">
                  <div class="icon">
                    <i class="flaticon-browser"></i>
                  </div>
                  <div class="content" style="color: #4760BB;">
                    <h2 style="font-weight: bolder; color: #4760BB;">Aula Virtual</h2>
                    <p style="font-weight: bolder;">Todo sobre nuestros cursos virtuales.</p>
                  </div>
                </a>
              </div>
            </div>
            <div class="col-md-3">
              <div class="left-icon-wraper">
                <a href="#">
                  <div class="icon"><i class="flaticon-browser-2"></i></div>
                  
                  <div class="content" style="color: #4760BB;">
                    <h2 style="font-weight: bolder; color: #4760BB;">Biblioteca virtual</h2>
                    <p style="font-weight: bolder;">Catálogo digital de nuestra biblioteca.</p>
                  </div>
                </a>    
              </div>
            </div>
            <div class="col-md-3">
              <div class="left-icon-wraper">
                <a href="#">
                  <div class="icon"><i class="flaticon-search-1"></i></div>
                  <div class="content" style="color: #4760BB;">
                    <h2 style="font-weight: bolder; color: #4760BB;">Repositorio digital</h2>
                    <p style="font-weight: bolder;">Catálogo de publicaciones e investigaciones.</p>
                  </div>
                </a>   
              </div>
            </div>
            <div class="col-md-3">
              <div class="left-icon-wraper">
                <a href="#">
                  <div class="icon"><i class="flaticon-viral-marketing"></i></div>
                  <div class="content" style="color: #4760BB;">
                    <h2 style="font-weight: bolder; color: #4760BB;">Plataforma estudiantil</h2>
                    <p style="font-weight: bolder;">Información académica y personal de nuestros estudiantes.</p>
                  </div>
                </a>   
              </div>
            </div>
        </div>
<!--         <div class="row">
            <div class="col-md-4">
                <div class="left-icon-wraper">
                <div class="icon"><i class="flaticon-blogging"></i></div>

                 <div class="content">
                     <h2>Content Marketing</h2>
                    <p>Efficiently monetize technically sound e-markets rather than interoperable e-services.</p>
                 </div>
                    
                </div>
            </div>
            <div class="col-md-4">
                <div class="left-icon-wraper">
                <div class="icon"><i class="flaticon-social-media"></i></div>

                 <div class="content">
                     <h2>Social Media</h2>
                    <p>Collaboratively seize best-of-breed manufactured products for inexpensive initiatives. Dynamically repurpose.</p>
                 </div>
                    
                </div>
            </div>
            <div class="col-md-4">
                <div class="left-icon-wraper">
                <div class="icon"><i class="flaticon-viral-marketing"></i></div>

                <div class="content">
                    <h2>Digital Marketing</h2>
                    <p>Enthusiastically promote standards compliant relationships vis-a-vis backend resources.</p>
                </div>
                   
                </div>
            </div>
        </div> -->
    </div>
</section>